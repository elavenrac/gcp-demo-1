...
class SplitPartitions(beam.DoFn):
    ...
    def start_bundle(self):
        try:
          subprocess.call('pip', 'install', '--user', 'tensorflow==1.13.*', 'tensorflow-transform==0.13.*', 'apache-beam[gcp]==2.11.*',
...
def run(argv=None, save_main_session=True):
    ...
    pipeline_args.extend([
     ...
     '--job_name=gcp-demo1-tf-etl-dos',
     '--setup_file=/Users/acarnevale/Projects/ntc-ml/setup.py',
     '--experiments=shuffle_mode=service',
     '--max_num_workers=4',
     '--worker_machine_type=n1-standard-4',
     #'--service_account_email=gcp-demo1@ml-sandbox-1-191918.iam.gserviceaccount.com',
     '--region=us-central1'])
    ...
    with beam.Pipeline(options=pipeline_options) as p:
        with tft_beam.Context('/tmp'):
            training_data = p | 'ReadBigQuery training' >> beam.io.Read(beam.io.BigQuerySource(query=("SELECT * FROM `{}.{}.{}` WHERE ml_partition='train'".format(p.options.project, known_args.dataset, known_args.table)),
              use_standard_sql=True))
            test_data = p | 'ReadBigQuery test' >> beam.io.Read(beam.io.BigQuerySource(query=("SELECT * FROM `{}.{}.{}` WHERE ml_partition='test'".format(p.options.project, known_args.dataset, known_args.table)),
              use_standard_sql=True))
            validation_data = p | 'ReadBigQuery eval' >> beam.io.Read(beam.io.BigQuerySource(query=("SELECT * FROM `{}.{}.{}` WHERE ml_partition='validation'".format(p.options.project, known_args.dataset, known_args.table)),
              use_standard_sql=True))
            transformed_train_dataset, transform_fn = (
             training_data, get_metadata()) | tft_beam.AnalyzeAndTransformDataset(preprocessing_fn)
            write_tfrecords(transformed_train_dataset, 'gs://{}/{}'.format(known_args.output_bucket, known_args.output_path), 'train')
            raw_test_dataset = (
             test_data, get_metadata())
            transformed_test_dataset = (
             raw_test_dataset, transform_fn) | '{} - Transform'.format('test') >> tft_beam.TransformDataset()
            write_tfrecords(transformed_test_dataset, 'gs://{}/{}'.format(known_args.output_bucket, known_args.output_path), 'test')
            raw_eval_dataset = (
             validation_data, get_metadata())
            transformed_eval_dataset = (
             raw_eval_dataset, transform_fn) | '{} - Transform'.format('validation') >> tft_beam.TransformDataset()
            write_tfrecords(transformed_eval_dataset, 'gs://{}/{}'.format(known_args.output_bucket, known_args.output_path), 'validation')
